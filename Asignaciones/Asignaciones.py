...
from Actualizar_pedidos import IniProcess
...
#settings
app.secret_key = "770071010"

#Creamos la variable global last_ofs para agilizar las querys individuales   
datosIni = readJson()
last_ofs = PostgresConn.Database(datosIni).select_ofs_by_date(14)

def getURLIp():
    datosIni = readJson()    
    url = "http://{}:{}".format(datosIni["ip"]["ipServer"], datosIni["ip"]["port"])
    return url

@app.route("/")
def Index():

    return render_template("asignaciones.html", url_asig = getURLIp())

@app.route("/getAsignacionesData")
...
@app.route("/getOfAsignacion", methods = ["POST","GET"])
def getOfAsignacion():
...
@app.route("/asignaciondirecta", methods = ["POST","GET"])
...
@app.route("/asignacionparcial", methods = ["POST","GET"])
...
@app.route("/editAsignacion", methods = ["POST","GET"])
def editAsignacion():
    column_names = ['error', 'id_pedido', 'id', 'status', 'originalquantity', 'destrio', 'description', 'linea_id', 'quantity']
    res = pd.DataFrame(columns = column_names)
...
@app.route("/deleteAsignacion", methods = ["POST","GET"])
...
if __name__ == '__main__':    
    sql = PostgresConn.Database(datosIni)      
    app.run(host= datosIni["ip2"]["host"], port=datosIni["ip2"]["port"], debug= False)
